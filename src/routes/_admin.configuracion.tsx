...
export const Route = createFileRoute("/_admin/configuracion")({ component: Config });

function Config() {
  const get = useServerFn(adminGetSettings);
  const upd = useServerFn(adminUpdateSettings);
  const { data, refetch } = useQuery({ queryKey: ["admin","settings"], queryFn: () => get() });
  ...
  async function save() {
    setSaving(true);
    try {
      await upd({ data: { admin_notification_email: email || null, send_customer_email: sendCustomer } });
      await refetch();
    ...
    <div className="max-w-xl space-y-5">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Se envía un correo aquí cada vez que un pedido es aprobado por Mercado Pago.</p>
