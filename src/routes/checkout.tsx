      ...
      <form onSubmit={handleSubmit} className="mt-8 grid gap-8 md:grid-cols-[1fr_340px]">
        ...
        <aside className="h-fit rounded-xl border border-border bg-card p-5 sticky top-20 space-y-4">
          ...
          <Button type="submit" disabled={loading} size="lg" className="w-full rounded-full">
            {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Redirigiendo al pago…</> : <><CreditCard className="mr-2 h-4 w-4"/>Pagar {formatMxn(subtotal)}</>}
          </Button>
          <p className="text-[11px] text-muted-foreground text-center">Pago seguro procesado por Mercado Pago · Tarjetas, OXXO, SPEI.</p>
